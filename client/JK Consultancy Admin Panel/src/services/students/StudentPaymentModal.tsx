...
interface StudentPaymentModalProps {
  ...
  sessionYearFilter: string; // Added to match filtered session year
  yearFilter: string; // Added to match filtered course year
  onClose: () => void;
}

const StudentPaymentModal: React.FC<StudentPaymentModalProps> = ({ studentId, students, sessionYearFilter, yearFilter, onClose }) => {
  const [paymentMethod, setPaymentMethod] = useState<'handover' | 'bank'>('handover');
 
  const student = students.find(s => s.id === studentId);
  if (!student) return null;
 
  // Find academic details matching the sessionYearFilter and yearFilter
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-3 gap-y-2">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <select className="w-full border border-gray-300 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-xs">
                      ...
                      <option value="AmountFees">Amount Fees</option>
                    </select>
                  </div>
                </div>
              ...
              <div className="p-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-0.5">
                      <label className="block text-xs font-medium text-gray-700">Upload bank Deposite / handover files</label>
                      <div className="relative">
