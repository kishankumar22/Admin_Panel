...
const AddStudentModal: React.FC<AddStudentModalProps> = ({ onClose, onSuccess, createdBy }) => {
  ...
  const handleTabClick = (tabNumber: number) => {
    // Validate before allowing tab switch
    if (tabNumber > step) {
      if (step === 1 && (!student.FName || !student.LName || !student.RollNumber)) {
        alert('Please complete all required fields in Personal Details before proceeding');
        return;
      }
      if (step === 2 && (!student.MobileNumber || !student.EmailId || !student.FatherMobileNumber)) {
        alert('Please complete all required fields in Contact Details before proceeding');
        return;
      }
      if (step === 3 && (!student.CourseName || !student.CourseYear || !student.Address)) {
        alert('Please complete all required fields in Academic & Address Details before proceeding');
        return;
      }
    }
    setStep(tabNumber);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm z-50">
      <div className="bg-white p-5 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex border-b mb-4">
          ...
          <button
            ...
            className={`px-4 py-2 font-medium ${step === 2 ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'} ${!student.FName || !student.LName || !student.RollNumber ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={!student.FName || !student.LName || !student.RollNumber}
          >
            Contact Details
          </button>
          <button
            onClick={() => handleTabClick(3)}
            className={`px-4 py-2 font-medium ${step === 3 ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'} ${!student.MobileNumber || !student.EmailId || !student.FatherMobileNumber ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={!student.MobileNumber || !student.EmailId || !student.FatherMobileNumber}
          >
            Academic & Address
          </button>
          <button
            onClick={() => handleTabClick(4)}
            className={`px-4 py-2 font-medium ${step === 4 ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'} ${!student.CourseName || !student.CourseYear || !student.CollegeId ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={!student.CourseName || !student.CourseYear || !student.CollegeId}
          >
            Final Details
          </button>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          {/* Step 2 Content - Contact Details */}
          {step === 2 && (
            <div className="animate-fadeIn">
          ...
          {/* Step 3 Content - Academic & Address Details */}
          {step === 3 && (
            <div className="animate-fadeIn">
          ...
          {/* Step 4 Content - Final & Administrative Details */}
          {step === 4 && (
            <div className="animate-fadeIn">
