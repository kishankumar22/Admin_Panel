  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        <input
          type="search"
          className='py-1 px-2 bg-white border border-gray-300 rounded-md text-xs w-80 placeholder:text-[.7rem] focus:outline-none focus:ring-4 focus:ring-blue-500 transition duration-200'
        ...
        <div className="flex items-center space-x-2">
          ...
          <Link to="/assign-page-to-role">
            <button className="bg-orange-500 text-white px-3 py-1 rounded text-xs hover:bg-orange-600 transition duration-200">
              <FaUser className="inline-block mr-1" /> {/* Assign Page to Role Icon */}
              Assign Page to Role
            </button>
          </Link>
        </div>
            ...
            filteredPages.map((page) => (
              <tr key={page.pageId} className="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td className="py-1 px-2 border-b">
                  <button
                    className={`bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600 transition duration-200 focus:outline-none focus:ring-4 focus:ring-green-300 ${!canUpdate ? 'opacity-50 cursor-not-allowed' : ''}`}
                    onClick={canUpdate ? () => {
                  ...
                  <button
                    className={`bg-red-500 text-white px-2 py-1 rounded ml-2 hover:bg-red-600 transition duration-200 focus:outline-none focus:ring-4 focus:ring-red-300 ${!canDelete ? 'opacity-50 cursor-not-allowed' : ''}`}
                    onClick={canDelete ? () => {
                      setSelectedPage(page);
                      setIsDeleteModalOpen(true);
                    } : () => toast.error('Access Denied: You do not have permission to delete pages.')}
                  >
                    <MdDelete className="inline-block mr-1" /> {/* Delete Icon */}
                    Delete
                  </button>
                </td>
              </tr>
