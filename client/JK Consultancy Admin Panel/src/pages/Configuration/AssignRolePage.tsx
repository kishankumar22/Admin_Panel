...
const AssignRolePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPages = async () => {
      try {
        const response = await axiosInstance.get('/pages'); // Adjust the URL according to your API
    ...
    <>
      ...
      <div className="flex justify-between items-center p-2 bg-gray-100 rounded-lg shadow-md dark:bg-gray-700">
        ...
        <div className="flex space-x-2 ml-auto">
          ...
          <button
            type="submit"
            className="bg-green-500 text-white px-3 py-1 rounded-md text-sm hover:bg-green-600 transition duration-200 focus:outline-none focus:ring-2 focus:ring-green-400"
          >
            Save
          </button>
        </div>
        ...
        <table className="min-w-full bg-white border border-gray-200">
          <thead>
            <tr className="bg-gray-200 text-gray-600 uppercase text-xs font-thin leading-normal">
              ...
              {roles.map((role) => <th key={role.role_id} className="px-2 py-1.5 text-left">
                  {role.name}
                </th>
              )}
            </tr>
          ...
          <tbody className="text-gray-600 text-sm font-light">
            {pages.map((page) => (
              <tr key={page.id} className="border-b text-xs h-14 border-gray-200 hover:bg-gray-100">
                        ...
                        <div
                          ...
                          style={{
                            top: buttonRefs.current[role.name]?.offsetHeight
                              ...
                              : 40,
                            left: buttonRefs.current[role.name]?.offsetLeft ?? 0,
                          }}
                        >
